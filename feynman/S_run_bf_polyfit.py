...
def run_bf_polyfit(filename,BF_try_time,BF_ops_file_type, PA, polyfit_deg=3, output_type=""):
    input_data = np.loadtxt(filename)
#############################################################################################################################
    if np.isnan(input_data).any()==False:
        # run BF on the data (+)
        print("Checking for brute force + \n")
        brute_force(filename,BF_try_time,BF_ops_file_type,"+")
    ...
    #############################################################################################################################
        ...
        try:
            polyfit_result = polyfit(polyfit_deg,filename)
            ...
            PA_poly = add_snap_expr_on_pareto( filename, str(eqn), PA_poly)
