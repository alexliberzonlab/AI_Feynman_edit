...
def check_translational_symmetry_minus(filename):
    try:
        pathdir_weights = "results/NN_trained_models/models/"

        # load the data
        n_variables = np.loadtxt("%s" %filename, dtype='str').shape[1]-1
        variables = np.loadtxt("%s" %filename, usecols=(0,))
        ...
        if n_variables==1:
            ...
            for j in range(1,n_variables):
                v = np.loadtxt("%s" %filename, usecols=(j,))
                variables = np.column_stack((variables,v))


        f_dependent = np.loadtxt("%s" %filename, usecols=(n_variables,))
...
def do_translational_symmetry_minus(filename, i,j):
    try:
        pathdir_weights = "results/NN_trained_models/models/"
        ...
        for k in range(1,n_variables):
            v = np.loadtxt("%s" %filename, usecols=(k,))
            variables = np.column_stack((variables,v))
...
# checks if f(x,y)=f(x/y)
def check_translational_symmetry_divide(filename):
    try:
        pathdir_weights = "results/NN_trained_models/models/"
...
def do_translational_symmetry_divide(filename, i,j):
    try:
        pathdir_weights = "results/NN_trained_models/models/"
...
# checks if f(x,y)=f(x*y)
def check_translational_symmetry_multiply(filename):
    try:
        pathdir_weights = "results/NN_trained_models/models/"
...
def do_translational_symmetry_multiply(filename, i,j):
    try:
        pathdir_weights = "results/NN_trained_models/models/"
...
# checks if f(x,y)=f(x+y)
def check_translational_symmetry_plus(filename):
    try:
        pathdir_weights = "results/NN_trained_models/models/"
...
def do_translational_symmetry_plus(filename, i,j):
    try:
        pathdir_weights = "results/NN_trained_models/models/"
