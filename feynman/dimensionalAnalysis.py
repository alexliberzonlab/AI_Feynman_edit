...
def dimensionalAnalysis(filename, eq_symbols):
    ...
    varibs = load_data(filename)[0]
    deps = load_data(filename)[1]
    ...
    if ok==0:
        dimless_data = load_data(filename)[0].T
        dimless_dep = load_data(filename)[1]
        if dimless_data.ndim==1:
            dimless_data = np.reshape(dimless_data,(1,len(dimless_data)))
            dimless_data = dimless_data.T
        np.savetxt(filename + "_dim_red", dimless_data)
        file_sym.write(", ")
        for j in range(len(input)):
        ...
        if new_vars.size==0:
            np.savetxt(filename + "_dim_red", new_dependent)

        # save this to file
        all_variables = np.vstack((new_vars, new_dependent)).T
        np.savetxt(filename + "_dim_red", all_variables)
